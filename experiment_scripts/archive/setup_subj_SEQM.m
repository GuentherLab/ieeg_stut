...
if find(strcmp({'a', 'b'}, pattern_learned)) ~= 0
    if find(strcmp({'a', 'b'}, pattern_novel)) ~= 0
        error('pattern_novel should not be "a" or "b".');
    elseif find(strcmp({'c', 'd'}, pattern_novel)) ~= 0
        pattern_prepost = 'EF';
    else
        pattern_prepost = 'CD';
    end
elseif find(strcmp({'c', 'd'}, pattern_learned)) ~= 0
    if find(strcmp({'c', 'd'}, pattern_novel)) ~= 0
        error('pattern_novel should not be "c" or "d".');
    elseif find(strcmp({'a', 'b'}, pattern_novel)) ~= 0
        pattern_prepost = 'EF';
    else
        pattern_prepost = 'AB';
    end
elseif find(strcmp({'e', 'f'}, pattern_learned)) ~= 0
    if find(strcmp({'e', 'f'}, pattern_novel)) ~= 0
        error('pattern_novel should not be "e" or "f".');
    elseif find(strcmp({'c', 'd'}, pattern_novel)) ~= 0
        pattern_prepost = 'AB';
    else
        pattern_prepost = 'CD';
...
switch pattern_learned
    ...
    case 'e'
        lidx = indices(mod(indices,6)>=1 & mod(indices,6)<=3);
        Lwords = stimuli.testEF;
        Trainwords = stimuli.trainEF;
    case 'f'
        lidx = indices(mod(indices,6)>=1 & mod(indices,6)<=3) + 15;
        lidx(lidx>30) = lidx(lidx>30)-30;
        Lwords = stimuli.testEF;
...
PPNwords = eval(sprintf('stimuli.test%s', pattern_prepost));
