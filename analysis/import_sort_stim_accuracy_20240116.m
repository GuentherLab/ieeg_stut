...
subsheet_name_string = {'_ABC'; 'ACB'; '_BAC'; '_BCA'; '_CAB'; '_CBA'}; % all subject sheets must start with this string
...
sub_tab_names = sort(sheettabs(contains(sheettabs, subsheet_name_string)));
...
for isub = 1:nsubs
    ...
    if all(strcmp('double',prepost_var_types) ) &&...
        ...
        % if subject table is missing fmri run variables, skip it
        if ~any(string(temptab.Properties.VariableNames) == fmri_run_var_names{1})
            fprintf(['skipping subject ' thissheetname ' because it is missing table variable ''' fmri_run_var_names{1} '''\n'])
            continue
        end

        % if this subject's fmri runs have not yet been scored (those variables are missing or empty), fill in with nans
