...
% legacy version of early-trial computation
n_early_trials_label_agnostic = 20; 
...
subtab = table({'pilot_001';'pilot_003';'pilot_004';'pilot_005'},[1;1;1;1],[1;1;1;1],{'ab';'ba';'ab';'ba'},       cell(nsubs,1),       cell(nsubs,1),...
    'VariableNames',{'sub',                         'sess',         'run',                'trainorder',   'trials_trainphase_a', 'trials_trainphase_b'});
...
for isub = 1:nsubs
    ...
    % label agnostic early trials
    trials_test_early = trials_test(1:n_early_trials_label_agnostic,:);
    subtab.acc_trained_early(isub) =  mean(trials_test_early{contains(trials_test_early.stim_group,{'test_a','test_b'}),performance_var});
    subtab.acc_novel_early(isub) = mean(trials_test_early{contains(trials_test_early.stim_group,{'test_novel'}),performance_var});
...
% %%% label-agnostic
[~,p_trained_vs_novel_all] = ttest(subtab.acc_trained,subtab.acc_novel);
[~,p_trained_vs_novel_early] = ttest(subtab.acc_trained_early,subtab.acc_novel_early);
